            ...
            <nav class="mt-6 px-3">
                <div class="space-y-1">
                    ...
                    <a href="{{ route('dashboard') }}" 
                       class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('dashboard') ? 'bg-primary-50 text-primary-700 border-r-2 border-primary-600' : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900' }}">
                        <i class="fas fa-tachometer-alt w-5 h-5 mr-3 {{ request()->routeIs('dashboard') ? 'text-primary-600' : 'text-gray-400 group-hover:text-gray-500' }}"></i>
                        Dashboard
                    </a>

                    <!-- Tasks -->
                    <a href="{{ route('tasks.index') }}" 
                       class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('tasks.*') ? 'bg-primary-50 text-primary-700 border-r-2 border-primary-600' : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900' }}">
                        <i class="fas fa-clipboard-list w-5 h-5 mr-3 {{ request()->routeIs('tasks.*') ? 'text-primary-600' : 'text-gray-400 group-hover:text-gray-500' }}"></i>
                        Tasks
                        @if(isset($taskCount) && $taskCount > 0)
                            <span class="ml-auto bg-primary-100 text-primary-600 text-xs font-medium px-2 py-0.5 rounded-full">{{ $taskCount }}</span>
                        @endif
                    </a>
                    ...
                    @if(auth()->user()->isAdmin())
                        ...
                        <a href="{{ route('users.index') }}" 
                           class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('users.*') ? 'bg-primary-50 text-primary-700 border-r-2 border-primary-600' : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900' }}">
                            <i class="fas fa-users w-5 h-5 mr-3 {{ request()->routeIs('users.*') ? 'text-primary-600' : 'text-gray-400 group-hover:text-gray-500' }}"></i>
                            Users
                        </a>
                    @endif
                </div>
